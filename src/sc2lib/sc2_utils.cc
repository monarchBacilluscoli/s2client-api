...
//todo check if the folder is existing, if not, create it, Attention: different system may need different method to do it
    ...
    {
        FILE *p_pipe = ::popen("pidof SC2_x64", "r");
        if (p_pipe == nullptr)
        {
    ...
    std::vector<std::string> splitStringByDelimter(std::string s, const std::string &delimiter)
    {
    ...
    {
        int status;
        std::string s = execCommand("pidof SC2_x64", status);
        auto results = splitStringByDelimter(s, " ");
        for (const auto &item : results)
        {
            kill(std::stoi(item), SIGKILL);
        }
    }
