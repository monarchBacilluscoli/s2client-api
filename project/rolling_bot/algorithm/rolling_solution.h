...
namespace sc2
{
    ...
    struct AverageSimData
    {
        ...
        static float CalculateAverWinRate(const std::vector<SimData> &results);
        static float CalculateAverEndLoop(const std::vector<SimData> &results);
        // static float CalculateTotalHealthLoss(const std::map<Tag, UnitStatisticalData> &stats, Unit::Alliance alliance);
    };
    ...
    void RollingSolution<T>::CalculateAver()
    {
        ...
        if (sim_sz > 1)
        {
            for (int i = 0; i + 1 < sim_sz; ++i) // to check if all the results have the same size of units statistical data
            {
