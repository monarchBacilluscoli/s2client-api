...
namespace sc2
{
void CheckUnitTagSame(const std::vector<Solution<Command>> &pop, const Solution<Command> *sol) // check if the order of units is the same with a solution in a population
{
    if (!sol)
    {
        sol = &(pop.front());
