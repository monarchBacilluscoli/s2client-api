...
namespace sc2 {
	...
	using population = std::vector<basic_solution>;
	...
	std::vector<basic_solution> random_bot::cross_over(const basic_solution& a, const basic_solution& b) {
		...
		if (a.size()) {
			int cross_over_point = GetRandomInteger(0, a.size() - 1);
	...
	void random_bot::evaluate_all_solutions(const population& p, std::vector<float>& total_damage, std::vector<float>& total_hurt) {
	...
	void random_bot::sort_solutions(population & p, std::vector<float>& total_damage, std::vector<float>& total_hurt) {
		// 对两个目标值逐个相减
	...
	basic_solution random_bot::select_solution() {
		...
		basic_solution selected_solution = m_population[0];
		int index(0);
	...
	basic_solution random_bot::select_random_solution() {
		return GetRandomEntry<population>(m_population);
	}
	...
	float random_bot::evaluate_single_solution_damage(const basic_solution & s) {
		float total_damage_make(0);
		std::map<const Unit*, float> damage_to_enemy;
		for (auto c : s) {
			const Unit* self = std::get<0>(c);
			const Unit* enemy = std::get<2>(c);
			if (enemy!=nullptr) {
				bool is_within_range = Distance2D(std::get<1>(c), enemy->pos) < Observation()->GetUnitTypeData()[self->unit_type].weapons[0].range;
				if (is_within_range) {
					damage_to_enemy[enemy] += damage_weapon_to_unit(Observation()->GetUnitTypeData()[self->unit_type].weapons[0], enemy);
				}
			}
		}
		// 处理过量伤害
	...
	float random_bot::evaluate_single_solution_hurt(const basic_solution & s) {
		...
		for (const Unit* u : enemy_units) {
			...
			float search_range = u_weapon.range;
			if (search_range < 1) {
				search_range = 1;
			}
	...
	Units random_bot::search_units_within_radius_in_solution(const Point2D& p, float r, const basic_solution& s) {
		Units satisfied_units;
		for (const command& c : s) {
			if (Distance2D(std::get<1>(c),p) <= r) {
				satisfied_units.push_back(std::get<0>(c));
			}
		}
		return satisfied_units;
	}
