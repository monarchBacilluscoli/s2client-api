...
namespace sc2 {
    ...
    private:
        ...
        // 评估当前所有解
        void evaluate_all_solutions(const population& p, std::vector<float>& total_damage, std::vector<float>& total_hurt);
        // 对解进行排序
        void sort_solutions(population& p, std::vector<float>& total_damage, std::vector<float>& total_hurt);
        // 根据评估值选择评估值最高的解
        ...
        // 评估我方输出伤害
        float evaluate_single_solution_damage(const basic_solution& s);
        // 评估我方受到伤害
        float evaluate_single_solution_hurt(const basic_solution& s);
        // 在解中搜索某点范围内友军
        Units search_units_within_radius_in_solution(const Point2D& p, float r, const basic_solution& s);
