...
namespace sc2 {
	...
	void potential_field_bot::OnGameStart() {
		...
		m_initial_units = Observation()->GetUnits();
		m_initial_self_units = Observation()->GetUnits(Unit::Alliance::Self);
		m_initial_enemy_units = Observation()->GetUnits(Unit::Alliance::Enemy);

		// initialize some data
		m_alive_self_units = m_initial_self_units;
		m_alive_enemy_units = m_initial_enemy_units;
		m_all_alive_units = m_initial_units;
