	...
	for (size_t i = 0; i < m_my_team.size(); i++)
	{
		...
		for (ActionRaw& action_raw : sol.variable[i].actions)
		{
			...
			if (GetRandomFraction() < m_attack_possibility) {
				...
				action_raw.target_type = ActionRaw::TargetType::TargetPosition; //? pay attention to here is my test code which need to changes
				action_raw.target_point += Point2DInPolar(GetRandomFraction() * moveable_radius, GetRandomFraction() * 2 * PI).toPoint2D();
			}
			else {
...
{
	//todo choose an action to mutate?
